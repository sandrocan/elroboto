...
#include "driver/uart.h"     // NEU: Hinzugefügt für die UART-Funktion
...
// NEU: UART Konfiguration (XIAO ESP32-C6)
#define UART_TX_PIN                 GPIO_NUM_16  // Hardware Pin D6
...
#define PROXIMITY_THRESHOLD_STOP    0.08f  // 4-5 cm Annäherung -> ROT
...
// ==============================================================================
// 3. HARDWARE INITIALISIERUNGEN (UART & Wi-Fi)
// ==============================================================================

// NEU: Konfiguration der UART Schnittstelle (nur Senden auf D6)
void init_uart(void) {
    ...
    // Setze Pin 16 (D6) als TX. RX wird nicht benötigt und deaktiviert (-1)
    ESP_ERROR_CHECK(uart_set_pin(UART_PORT_NUM, UART_TX_PIN, UART_PIN_NO_CHANGE, UART_PIN_NO_CHANGE, UART_PIN_NO_CHANGE));
    ESP_LOGI(TAG, "UART initialisiert. Sende Telemetrie auf TX Pin %d (D6)", UART_TX_PIN);
}
...
void collision_processing_task(void *pvParameters) {
    ...
    int uart_throttle_counter = 0; // Zähler zum Drosseln der UART-Nachrichten
    ...
    while (1) {
        if (xQueueReceive(eskin_data_queue, &rx_pkt, portMAX_DELAY) == pdTRUE) {
            ...
            if (rx_pkt.payload[0] == 0xFF) {
                ...
                // --- NEU: UART TEST SENDEN ---
                uart_throttle_counter++;
                // Wir senden nur jedes 3. Paket an den STM32, um ihn nicht zu überlasten
                if (uart_throttle_counter % 3 == 0) {
                    char tx_buffer[32];
                    
                    // TEST-MODUS: Sendet stur "0.555" (Mit Newline für den STM32-Parser!)
                    int len = snprintf(tx_buffer, sizeof(tx_buffer), "0.555\n");
                    
                    // WENN DER TEST KLAPPT, LÖSCHST DU DIE ZEILE OBEN UND NUTZT DIESE HIER:
                ...
                // State-Machine für LEDs und STM32
                if (prox_val > PROXIMITY_THRESHOLD_TOUCH) {
                    if (current_color != 2 && global_udp_sock != -1) {
                        ...
                        ESP_LOGW(TAG, "BERÜHRUNG! LED = BLAU (Prox: %.3f)", prox_val);
                    }
                } 
                else if (prox_val > PROXIMITY_THRESHOLD_STOP) {
                    if (current_color != 1 && global_udp_sock != -1) {
                        ...
                        current_color = 1;
                        gpio_set_level(COLLISION_GPIO, 1); 
                        ESP_LOGW(TAG, "Kollision droht! LED = ROT (Prox: %.3f)", prox_val);
                    }
                } 
                else if (prox_val < PROXIMITY_THRESHOLD_CLEAR) {
                    if (current_color != 0 && global_udp_sock != -1) {
                        ...
                        current_color = 0;
                        gpio_set_level(COLLISION_GPIO, 0);
                        ESP_LOGI(TAG, "Weg frei. LED = GRÜN (Prox: %.3f)", prox_val);
                    }
                }
            }
        }
...
void app_main(void) {
    ...
    init_uart(); // NEU: Initialisiert die serielle Schnittstelle
