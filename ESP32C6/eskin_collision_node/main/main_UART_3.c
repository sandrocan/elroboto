...
#include "driver/uart.h"     // NEU: Für UART hinzugefügt
#include "driver/gpio.h"     // NEU: Für Pin-Definition hinzugefügt
...
#define PROXIMITY_THRESHOLD_STOP    0.03f  // Annäherung -> ROT
#define PROXIMITY_THRESHOLD_CLEAR   0.016f  // Weg frei -> GRÜN
...
// ==============================================================================
// 2. HARDWARE INITIALISIERUNG (WLAN & UART)
// ==============================================================================
...
void init_uart(void) {
    ...
    ESP_LOGI(TAG, "UART initialisiert. Sende Daten auf TX Pin %d", UART_TX_PIN);
}
...
// ==============================================================================
// 3. DATENVERARBEITUNG (LEDs, Terminal Print & UART Senden)
// ==============================================================================
void collision_processing_task(void *pvParameters) {
    ...
    // Timer für die 4Hz Ausgabe (Terminal & UART)
    ...
    while (1) {
        if (xQueueReceive(eskin_data_queue, &rx_pkt, portMAX_DELAY) == pdTRUE) {
            if (rx_pkt.payload[0] == 0xFF) {
                ...
                // ==================================================
                // 4. TERMINAL & UART AUSGABE (Exakt alle 250ms -> 4 Hz)
                // ==================================================
                TickType_t current_time = xTaskGetTickCount();
                if ((current_time - last_print_time) >= pdMS_TO_TICKS(250)) {
                    last_print_time = current_time;
                    
                    // a) Schreibt den Wert sauber und flüssig in dein VS Code Terminal
                    printf("Max Prox: %.3f\n", max_prox);

                    // b) NEU: Schickt exakt denselben Wert über das UART-Kabel!
                    char tx_buffer[32];
...
void app_main(void) {
    ...
    init_uart(); // NEU: Aktiviert den UART-Port
